...
const level = 5;  //change level here
...
router.get('/:answer', authenticatedMiddleware(), onAnswer.currentLevel(level), onAnswer.insertIntoLog(level), function (req, res, next) {
    ...
    if (req.params.answer.toLowerCase() === 'manipalmanipal'|| req.params.answer.toLowerCase() === 'manipal') { //answer
        onAnswer.updateUserLevel(uid, level+1)
        .then(function() {
            res.redirect('/ryandahl'); //change redirection link
        })
