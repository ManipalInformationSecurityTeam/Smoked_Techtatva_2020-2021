...
const level = 28;  //change level here !important
...
router.get('/:answer', authenticatedMiddleware(), onAnswer.currentLevel(level), onAnswer.insertIntoLog(level), function (req, res, next) {
    ...
    if (req.params.answer.toLowerCase() === 'applerecords' || req.params.answer.toLowerCase() === 'applerecord' ) { //answer
        onAnswer.updateUserLevel(uid, level+1)
        .then(function() {
            res.redirect('/windmill'); //change redirection link
        })
