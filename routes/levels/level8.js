...
const level = 8; //change level here
...
router.get(
  ...
  authenticatedMiddleware(),
  onAnswer.currentLevel(level),
  onAnswer.insertIntoLog(level),
  function (req, res, next) {
    const uid = req.session.passport.user.user_id;
    if (req.params.answer.toLowerCase() === "reddeadredemption") {
      //answer
      onAnswer
        .updateUserLevel(uid, level + 1)
        .then(function () {
          res.redirect("/v.i"); //change redirection link
        })
